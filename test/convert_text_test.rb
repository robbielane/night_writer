...
class ConvertTextTest < Minitest::Test
  def test_night_write_outputs_file_sucessfully
    assert_equal "Created 'test/braille.txt' containing 68 characters",
                `ruby ./lib/night_write.rb test/message.txt test/braille.txt`.chomp
    assert $?.success?
  end
  ...
  def test_converts_string_of_numbers_to_braille
